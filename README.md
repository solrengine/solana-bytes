...
- 102 tests covering decoders, controllers, presenters, and game integrity
...
7. Results saved to leaderboard if connected via wallet. The save endpoint:
   - Requires the token's `user_id` to match the authenticated user (no token donation)
   - Rejects tokens older than 10 minutes (no replay)
   - Is idempotent via a SHA256 digest of the token (unique-indexed) — Turbo retries don't duplicate rows
...
**Token-2022 multisig accounts** — The decoder dispatches by account size, so Token-2022 multisig accounts that happen to share a length with a regular Token Account may render with incorrect field labels. Visual-only; the raw bytes are still correct. Rare in the wild and not in the challenge pool.

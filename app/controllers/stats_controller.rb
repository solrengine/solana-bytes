class StatsController < ApplicationController
  ...
  def show
    @stats = Rails.cache.fetch("stats:public", expires_in: 5.minutes, race_condition_ttl: 30.seconds) do
      {
        # Visit stats
        ...
        # NOTE: json_extract is SQLite-specific. Update if migrating to PostgreSQL.
        ...
        total_stars: ChallengeResult.sum(:total_stars),
        ...
        # Top players
        top_players: ChallengeResult.select("user_id, MAX(streak) as best_streak, COUNT(*) as games, SUM(total_stars) as total_stars")
                                     .group(:user_id)
  ...
  def normalized_countries
    ...
    raw_countries.each do |country, count|
      name = country.length == 2 ? (CountryCodes::CODE_TO_NAME[country.upcase] || country) : country
      merged[name] += count
    end

class AccountPresenter
  ...
  # True when at least one region has been identified beyond the catchall
  # "Data" / "Program Bytecode" fallback. Used to surface a "not decoded yet"
  # notice for accounts owned by programs we don't yet decode.
  def decoded?
    regions.any? { |r| r.id != "data" }
  end
  ...
  def hex_rows
    @hex_rows ||= raw_bytes.each_slice(BYTES_PER_ROW).each_with_index.map do |row_bytes, index|

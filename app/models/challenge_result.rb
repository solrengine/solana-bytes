class ChallengeResult < ApplicationRecord
  ...
  validates :account_address, :target_field, :time_seconds, :attempts, :total_stars, :streak, presence: true
  validates :account_address, format: { with: /\A[1-9A-HJ-NP-Za-km-z]{32,44}\z/, message: "must be a valid base58 address" }
  validates :total_stars, numericality: { greater_than_or_equal_to: 0, less_than_or_equal_to: 600 }
  ...
  scope :leaderboard, -> { order(streak: :desc) }

...
class RegionDecoderTest < ActiveSupport::TestCase
  ...
  def build_alt_bytes(deactivation_slot: ALT_ACTIVE_SLOT, last_extended_slot: 100, start_index: 0, authority: nil, addresses: [])
    bytes = []
    bytes += [ 1, 0, 0, 0 ]                              # discriminator = 1 (LookupTable)
    bytes += [ deactivation_slot ].pack("Q<").bytes      # u64 LE
    bytes += [ last_extended_slot ].pack("Q<").bytes     # u64 LE
    bytes << start_index                                  # u8
    if authority
      bytes << 1                                          # authority option = Some
      bytes += authority                                  # 32 bytes
    else
      bytes << 0                                          # authority option = None (frozen)
      bytes += [ 0 ] * 32                                 # zero pubkey
    end
    bytes += [ 0, 0 ]                                     # padding (u16)
    addresses.each { |addr| bytes += addr }               # variable-length array
    bytes
  end

  test "address lookup table decodes header with authority" do
    addr1 = (1..32).to_a
    addr2 = (33..64).to_a
    addr3 = (65..96).to_a
    bytes = build_alt_bytes(
      deactivation_slot: ALT_ACTIVE_SLOT,
      last_extended_slot: 200_000_000,
      start_index: 5,
      authority: (100..131).to_a,
      addresses: [ addr1, addr2, addr3 ]
    )

    regions = RegionDecoder.decode(ALT_OWNER, bytes)

    assert_equal "LookupTable", regions.find { |r| r.id == "lut_discriminator" }.decoded_value
    assert_equal "Active (max u64)", regions.find { |r| r.id == "lut_deactivation_slot" }.decoded_value
    assert_equal "200000000", regions.find { |r| r.id == "lut_last_extended_slot" }.decoded_value
    assert_equal "5", regions.find { |r| r.id == "lut_start_index" }.decoded_value
    assert_equal "Some", regions.find { |r| r.id == "lut_auth_option" }.decoded_value

    auth_region = regions.find { |r| r.id == "lut_authority" }
    assert_equal 22, auth_region.start
    assert_equal 32, auth_region.length
    refute_includes auth_region.decoded_value, "frozen"
  end

  test "address lookup table marks frozen tables when authority is None" do
    bytes = build_alt_bytes(authority: nil, addresses: [])
    regions = RegionDecoder.decode(ALT_OWNER, bytes)

    assert_equal "None", regions.find { |r| r.id == "lut_auth_option" }.decoded_value
    auth_region = regions.find { |r| r.id == "lut_authority" }
    assert_includes auth_region.decoded_value, "frozen"
    assert_equal "gray", auth_region.color
  end

  test "address lookup table decodes deactivated tables with concrete slot" do
    bytes = build_alt_bytes(deactivation_slot: 250_000_000, addresses: [])
    regions = RegionDecoder.decode(ALT_OWNER, bytes)

    deact = regions.find { |r| r.id == "lut_deactivation_slot" }
    assert_equal "250000000", deact.decoded_value
  end

  test "address lookup table emits one region per 32-byte address with shared id" do
    addr1 = Array.new(32) { 0xAA }
    addr2 = Array.new(32) { 0xBB }
    addr3 = Array.new(32) { 0xCC }
    bytes = build_alt_bytes(addresses: [ addr1, addr2, addr3 ])

    regions = RegionDecoder.decode(ALT_OWNER, bytes)
    address_regions = regions.select { |r| r.id == "lookup_address" }

    assert_equal 3, address_regions.length
    assert_equal [ 56, 88, 120 ], address_regions.map(&:start)
    assert(address_regions.all? { |r| r.length == 32 })
    assert(address_regions.all? { |r| r.name == "Lookup Address" })
    assert_match(/^#1: /, address_regions[0].decoded_value)
    assert_match(/^#3: /, address_regions[2].decoded_value)
  end

  test "address lookup table includes padding region" do
    bytes = build_alt_bytes(addresses: [])
    regions = RegionDecoder.decode(ALT_OWNER, bytes)
    padding = regions.find { |r| r.id == "lut_padding" }
    assert_equal 54, padding.start
    assert_equal 2, padding.length
  end

  test "address lookup table returns empty for too-short bytes" do
    short_bytes = [ 1, 0, 0, 0 ] + [ 0 ] * 30 # only 34 bytes, less than 56-byte header
    assert_equal [], RegionDecoder.send(:decode_address_lookup_table, short_bytes)
  end

  test "address lookup table region descriptions are populated" do
    bytes = build_alt_bytes(authority: (1..32).to_a, addresses: [ (33..64).to_a ])
    regions = RegionDecoder.decode(ALT_OWNER, bytes)

    deact = regions.find { |r| r.id == "lut_deactivation_slot" }
    assert_includes deact.description, "u64::MAX"

    addr_region = regions.find { |r| r.id == "lookup_address" }
    assert_includes addr_region.description, "v0 transactions"
  end

  # --- SPL Token Multisig tests ---

  TOKEN_OWNER = "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA"
  TOKEN_2022_OWNER = "TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb"

  def build_multisig_bytes(m: 2, n: 3, is_initialized: 1, signers: [])
    bytes = []
    bytes << m
    bytes << n
    bytes << is_initialized
    11.times do |i|
      if i < signers.length
        raise "Each signer must be 32 bytes" unless signers[i].length == 32
        bytes += signers[i]
      else
        bytes += [ 0 ] * 32
      end
  ...
  test "multisig decodes m, n, and is_initialized" do
    bytes = build_multisig_bytes(m: 2, n: 3, is_initialized: 1, signers: [ Array.new(32) { 1 }, Array.new(32) { 2 }, Array.new(32) { 3 } ])
    regions = RegionDecoder.decode(TOKEN_OWNER, bytes)

    assert_equal "2", regions.find { |r| r.id == "multisig_m" }.decoded_value
    assert_equal "3", regions.find { |r| r.id == "multisig_n" }.decoded_value
    assert_equal "Yes", regions.find { |r| r.id == "multisig_is_initialized" }.decoded_value
  end

  test "multisig populates active signer slots and marks empty ones" do
    bytes = build_multisig_bytes(m: 1, n: 2, signers: [ Array.new(32) { 0xAA }, Array.new(32) { 0xBB } ])
    regions = RegionDecoder.decode(TOKEN_OWNER, bytes)

    active = regions.select { |r| r.id == "multisig_signer" }
    empty = regions.select { |r| r.id == "multisig_signer_empty" }

    assert_equal 2, active.length
    assert_equal 9, empty.length
    assert_equal [ 3, 35 ], active.map(&:start)
    assert(active.all? { |r| r.length == 32 })
    assert(active.all? { |r| r.decoded_value.length > 30 })  # base58 pubkey
    assert_equal "green", active.first.color
    assert_equal "gray", empty.first.color
  end

  test "multisig decoder fires for Token-2022 owner too" do
    bytes = build_multisig_bytes(signers: [ Array.new(32) { 1 } ])
    regions = RegionDecoder.decode(TOKEN_2022_OWNER, bytes)
    assert regions.any? { |r| r.id == "multisig_m" }
  end

  test "multisig is exactly 355 bytes" do
    bytes = build_multisig_bytes
    assert_equal 355, bytes.length
  end

  test "multisig decoder rejects non-355-byte input" do
    short = [ 1, 2, 1 ] + Array.new(50, 0)
    assert_equal [], RegionDecoder.send(:decode_spl_multisig, short)
  end

  test "multisig routing does not interfere with 165-byte token accounts" do
    # 165-byte account should still decode as token account, not multisig
    fake_token_bytes = Array.new(165) { |i| i % 256 }
    regions = RegionDecoder.decode(TOKEN_OWNER, fake_token_bytes)
    assert regions.any? { |r| r.id == "mint" }, "165-byte account should route to decode_spl_token"
    refute regions.any? { |r| r.id == "multisig_m" }, "165-byte account must not be decoded as multisig"
  end

  test "multisig routing does not interfere with 82-byte mints" do
    bytes = build_spl_mint_bytes
    regions = RegionDecoder.decode(USDC_MINT_OWNER, bytes)
    assert regions.any? { |r| r.id == "mint_authority" }
    refute regions.any? { |r| r.id == "multisig_m" }
  end
end

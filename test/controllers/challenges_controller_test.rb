...
class ChallengesControllerTest < ActionDispatch::IntegrationTest
  ...
  def valid_token(overrides = {})
    ...
    }.merge(overrides), purpose: :challenge_progress)
  end
  ...
  test "challenge verifier round-trips data correctly" do
    ...
    token = challenge_verifier.generate(data, purpose: :challenge_progress)
    result = challenge_verifier.verify(token, purpose: :challenge_progress)
  ...
  test "tampered token raises InvalidSignature" do
    ...
    assert_raises(ActiveSupport::MessageVerifier::InvalidSignature) do
      challenge_verifier.verify(tampered, purpose: :challenge_progress)
    end
  end

  test "next challenge token carries streak forward" do
    token = next_challenge_token(streak: 5, total_stars: 12)
    data = challenge_verifier.verify(token, purpose: :challenge_progress)
  ...
  test "ChallengeResult rejects total_stars over 600" do
    ...
    assert result.errors[:total_stars].any?
  end
  ...
  test "ChallengeResult accepts valid data" do
    ...
    assert_empty result.errors[:total_stars]
